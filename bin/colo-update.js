...
async function updateAll() {
    updateFiles()
    await updateDeps('--save', 'brown-ui')
    await updateDeps('-g', 'brown-cli')
}
...
async function updateFiles() {
    ...
    function loadList() {
        ...
        try {
            colorc = require(resolveCwd('temp/.colorc.js'))
        } catch (err) {
        ...
        if (!config) return colorc.updateList

        const newUpdateList = colorc.updateList
        const oldUpdateList = config.updateList

        if (oldUpdateList && !Array.isArray(oldUpdateList))
            throw new Error('.colorc 中的 ignore 选项必须是数组类型')

        if (colorc && newUpdateList && Array.isArray(newUpdateList)) {
            if (colorc._meta && config._meta) {
                if (colorc._meta.type !== config._meta.type) return []

                const newVersion = semver.valid(colorc._meta.version)
